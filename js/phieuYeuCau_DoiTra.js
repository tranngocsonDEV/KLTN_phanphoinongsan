$(document).ready(function () {
  ...
  var table = $("#example").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: "../php/phieuYeuCau_DoiTra_HienThi.php",
      type: "POST",
      data: function (d) {
        d.action = "fetchData";
    ...
    columns: [
      ...
      { data: "NgayLapDoiTra" },
      {
        ...
        render: function (data, type, row) {
          let actionBtns = `
            <a href='./phieuYeuCau_ChiTietDoiTra_HienThi.php?MaYeuCauDoiTra=${row.MaYeuCauDoiTra}' class='btn btn-primary btn-md h-100 fs-6'>Xem</a>
            `;

          if ($("#trangThaiSelect").val() == 18) {
            // Trạng thái "Chờ duyệt"
            actionBtns = `
                    <a href='./phieuYeuCau_ChiTietDoiTra_DuyetPhieu.php?MaYeuCauDoiTra=${row.MaYeuCauDoiTra}' class='btn btn-warning btn-md h-100 fs-6 mt-1' style="padding-right: 1rem; ">Xem</a>`;
          }
          return actionBtns;
        }
      }
