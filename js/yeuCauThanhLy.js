$(document).ready(function () {
  ...
  var table = $("#example").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: "../php/yeuCau_ThanhLy_HienThi.php",
      type: "POST",
      data: function (d) {
        d.action = "fetchData";
    ...
    columns: [
      ...
      { data: "ThoiGian" },
      {
        ...
        render: function (data, type, row) {
          let actionBtns = `<a href='./yeuCau_ThanhLy_HienThi.php?MaThanhLy=${row.MaThanhLy}' class='btn btn-primary btn-md h-100 fs-6'>Xem</a>`;

          if ($("#trangThaiSelect").val() == 17) {
            actionBtns = `
                <a href='./yeuCau_ThanhLy_DuyetPhieu.php?MaThanhLy=${row.MaThanhLy}' class='btn btn-warning btn-md h-100 fs-6 mt-1' style="padding-right: 1rem; ">Xem</a>`;
          }
          return actionBtns;
        }
      }
