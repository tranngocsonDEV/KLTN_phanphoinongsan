$(document).ready(function () {
  ...
  var table = $("#example").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: "../php/nvPYC_DoiTra_HienThi.php",
      type: "POST",
      data: function (d) {
        d.action = "fetchData";
        d.status = $("#trangThaiSelect").val() || 19; // Mặc định "Đã duyệt" nếu chưa chọn trạng thái
      }
    },
    ...
    columns: [
      ...
      { data: "NgayLapDoiTra" },
      {
        ...
        render: function (data, type, row) {
          let actionBtns = `<a href='./nvPYC_DoiTra_ChiTiet.php?MaYeuCauDoiTra=${row.MaYeuCauDoiTra}' class='btn btn-primary btn-md h-100 fs-6'>Xem</a>`;

          if ($("#trangThaiSelect").val() == 18) {
            actionBtns += ` <a href='./nvPYC_DoiTra_ChiTiet_Sua.php?MaYeuCauDoiTra=${row.MaYeuCauDoiTra}' class='btn btn-warning btn-md h-100 fs-6 mt-1' style="padding-right: 1rem;">Sửa</a>
                              <a href='#' data-maphieu='${row.MaYeuCauDoiTra}' class='btn btn-danger btn-md h-100 fs-6 mt-1 btn_xoa_phieu_nhap' style="padding-right: 1rem;">Xóa</a>`;
          }
          return actionBtns;
        }
      }
