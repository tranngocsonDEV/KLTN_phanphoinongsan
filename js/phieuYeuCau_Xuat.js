$(document).ready(function () {
  ...
  var table = $("#example").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: "../php/phieuYeuCau_Xuat_HienThi.php",
      type: "POST",
      data: function (d) {
        d.action = "fetchData";
    ...
    columns: [
      ...
      { data: "TongSoLoaiSanPham" },
      {
        ...
        render: function (data, type, row) {
          let actionBtns = `
          <a href='./phieuYeuCau_ChiTietXuat_HienThi.php?MaPhieuXuatKho=${row.MaPhieuXuatKho}' class='btn btn-primary btn-md h-100 fs-6'>Xem</a>
          `;

          if ($("#trangThaiSelect").val() == 3) {
            // Trạng thái "Chờ duyệt"
            actionBtns = `
                  <a href='./phieuYeuCau_ChiTietXuat_DuyetPhieu.php?MaPhieuXuatKho=${row.MaPhieuXuatKho}' class='btn btn-warning btn-md h-100 fs-6 mt-1' style="padding-right: 1rem; ">Xem</a>`;
          }
          return actionBtns;
        }
      }
