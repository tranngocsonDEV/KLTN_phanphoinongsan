$(document).ready(function () {
  ...
  var table = $("#example").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: "../php/nvPhieuYeuCau_HienThi.php",
      type: "POST",
      data: function (d) {
        d.action = "fetchData";
    ...
    columns: [
      ...
      { data: "TongSoLoaiSanPham" },
      {
        ...
        render: function (data, type, row) {
          let actionBtns = `<a href='./nvPhieuYeuCau_Nhap_ChiTiet.php?MaPhieuNhapKho=${row.MaPhieuNhapKho}' class='btn btn-primary btn-md h-100 fs-6'>Xem</a>`;

          if ($("#trangThaiSelect").val() == 1) {
            actionBtns += ` <a href='./nvphieuYeuCau_Nhap_ChiTiet_Sua.php?MaPhieuNhapKho=${row.MaPhieuNhapKho}' class='btn btn-warning btn-md h-100 fs-6 mt-1' style="padding-right: 1rem;">Sửa</a>
                            <a href='#' data-maphieu='${row.MaPhieuNhapKho}' class='btn btn-danger btn-md h-100 fs-6 mt-1 btn_xoa_phieu_nhap' style="padding-right: 1rem;">Xóa</a>`;
          }
          return actionBtns;
        }
      }
  ...
  let MaPhieuNhapKho; // Lưu trữ mã phiếu nhập để xóa

  $(document).on("click", ".btn_xoa_phieu_nhap", function (e) {
    e.preventDefault();
    MaPhieuNhapKho = $(this).data("maphieu"); // Lấy mã phiếu nhập từ data-maphieu
  ...
  $("#confirmDeleteBtn").on("click", function () {
    $.ajax({
      url: "../view/nvphieuYeuCau_Nhap_ChiTiet_Xoa.php",
      type: "POST",
      data: { MaPhieuNhapKho: MaPhieuNhapKho },
      dataType: "json",
