...
$sql = "SELECT px.MaPhieuXuat, lp.Ten AS TenPhieu, u.Ten AS TenNguoiLapPhieu, px.NgayGiaoHang, px.TongSoLoaiSanPham 
...
while ($row = $result->fetch_assoc()) {
    ...
    if ($status == "8") {
        $actionBtn = "<a href='./phanPhoiDonHang_Ship_HienThi_HoanThanh.php?MaPhieuXuat=" . $row['MaPhieuXuat'] . "' class='btn btn-primary btn-md h-100 fs-6'>Xem</a>";
    } else {
        // Nếu là trạng thái khác (ví dụ, Chờ phân phối), hiển thị nút khác (hoặc không hiển thị gì)
        $actionBtn = "<button class='btn btn-primary btn-md' onclick=\"window.location='phanPhoiDonHang_Ship_HienThi_HoanThanh.php?MaPhieuXuat=" . $row['MaPhieuXuat'] . "'\">Xem</button>";
    }
