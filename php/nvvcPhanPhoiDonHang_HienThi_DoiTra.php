...
$sql = "SELECT
 DISTINCT yc.MaYeuCauDoiTra, yc.MaPhieuXuat,lp.Ten AS TenPhieu, u.Ten AS TenNguoiLapPhieu, yc.NgayLapDoiTra
                       FROM tbl_yeucaudoitra AS yc
                       JOIN tbl_chitietyeucaudoitra AS ct 
                       ...
                       JOIN tbl_user u ON u.IDNhanVien = yc.NguoiLapDoiTra
                        WHERE yc.MaTrangThaiPhieu = ? AND ct.MaShipper =  ?";
...
while ($row = $result->fetch_assoc()) {
    ...
    if ($status == "8") {
        ...
        $actionBtn = "<a href='./nvvcPhanPhoiDonHang_Ship_HienThi_DoiTra.php?MaPhieuXuat=" . $row['MaPhieuXuat'] . "&MaYeuCauDoiTra=" . $row['MaYeuCauDoiTra'] . "' class='btn btn-primary btn-md h-100 fs-6'>Xem</a>";
    } else {
        // Nếu là trạng thái khác (ví dụ, Chờ phân phối), hiển thị nút khác (hoặc không hiển thị gì)
        $actionBtn = "<button class='btn btn-primary btn-md' onclick=\"window.location='nvvcPhanPhoiDonHang_Ship_HienThi_HoanThanh.php?MaPhieuXuat=" . $row['MaPhieuXuat'] . "&MaYeuCauDoiTra=" . $row['MaYeuCauDoiTra'] . "'\">Xem</button>";
    }
